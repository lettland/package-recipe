...
final class Executor
{
    ...
    public function __construct(
        ...
        private State $state,
        #[TaggedLocator(Merger\Merger::class, defaultIndexMethod: 'getName')]
        private ServiceLocator $mergers,
    ) {
    }

    public function runRecipes(?array $packages = null, bool $overwrite = false): void
    {
        ...
        foreach ($this->getRequiredPackages($packages) as $package) {
            ...
            if (false === $this->executePackageRecipe()) {
    ...
    public function uninstallRecipes(array $packages): void
    {
        ...
        foreach ($packages as $package) {
            ...
            if (false === $this->uninstallPackageRecipe()) {
    ...
    private function executePackageRecipe(): ?bool
    {
        ...
        $this->io->write(sprintf("\nFound recipe for <comment>%s</>", $this->state->getCurrentPackage()));
    ...
    private function uninstallPackageRecipe(): ?bool
    {
        ...
        $this->io->write(sprintf("\nUninstalling recipe for <comment>%s</>", $this->state->getCurrentPackage()));
    ...
    private function loadRecipe(): ?array
    {
        ...
        if ($isYamlRecipe && !in_array('symfony/yaml', InstalledVersions::getInstalledPackages(), true)) {
    ...
    private function setRecipeDefaults(?array $recipe): array
    {
        $resolver = (new OptionsResolver())
            ->setDefaults([
                'files' => static function (OptionsResolver $filesResolver): void {
                    $filesResolver
                        ->setPrototype(true)
                        ->setDefined([
                            'type',
